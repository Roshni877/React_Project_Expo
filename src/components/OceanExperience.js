...
import { Points, PointMaterial, Float, Text, MeshDistortMaterial, PerspectiveCamera } from "@react-three/drei";
import * as THREE from "three";

// GLSL SHADER FOR OCEAN WAVES
const vertexShader = `
  varying vec2 vUv;
  ...
  void main() {
    vUv = uv;
    float elevation = pnoise(vec3(position.xy * uFrequency, uTime * uSpeed), vec3(10.0)) * 0.5;
...
function WaterSurface() {
  ...
  return (
    <mesh ref={meshRef} rotation={[-Math.PI / 2.5, 0, 0]} position={[0, -1, -2]}>
...
function ParticleSpray() {
  ...
  useFrame((state) => {
    ...
    pointsRef.current.position.z = Math.sin(time * 0.2) * 2;
  });
...
function OceanExperience() {
  return (
    <div className="ocean-experience-canvas">
      <Canvas dpr={[1, 2]}>
        ...
        <fog attach="fog" args={["#00050a", 5, 15]} />
      </Canvas>
    </div>
  );
}
